...
weight_dens = data.frame(weight = density(sbp$weight)$x, density = density(sbp$weight)$y)
...
There are packages with functions to calculate modes (e.g., [`modeest`](https://cran.r-project.org/web/packages/modeest/index.html)), but they come with an additional complexity that is usually not necessary for typical analytic needs.  
...
knitr::kable(round(describe(sbp[ , sapply(sbp, is.numeric)], skew = FALSE), 1))
```
...
3. (*optional*) Create an "advanced" *Table 1* from your own data or from the neurosurgery data linked in question 1, using `Phase` as the strata. [Email it to Dwight](mailto:dwight.barry@seattlechildrens.org?Subject=TableOne) if you make your own and want to have it shared next class for participant feedback (either anonymously or not, your choice).    
...
Let's look at this idea by returning to our original Stats Canada case study data, and take the study group as a whole, instead of being stratified into *control* and *treatment* groups.  
