...
Once we have a thorough understanding of each variable, we're ready to explore the possible relationships between variables---usually two or three variables (sometimes 4) can be visualized with traditional visualizations like bar plots and scatter plots.  
...
For the purposes of training, we started with univariate data exploration, but in reality you should do some basic overall assessments of your data as a whole at the start---specifically, explore missing or suspcious values.  

We'll create a second data set from the original, and mess with it a little to show what that might look like. 
...
bad_sbp$age[is.na(bad_sbp$age)] = sample(95:120, sum(is.na(bad_sbp$age)), replace = TRUE)
```
...
```{r, fig.height = 5.75, fig.width = 8, cache = TRUE}
matrixplot(bad_sbp)
```
...
Sometimes factors don't convert like you'd expect, so it's always worth double-checking to ensure that the original variable and the new variable line up exacty.  

```{r}
table(sbp$Gender, sbp$gender)
```

Now step through each new factor to ensure the the assignments were correct (output not shown in this document).  

```{r eval = FALSE}
table(sbp$Married, sbp$married)

table(sbp$Smoke, sbp$smoke)

table(sbp$Exercise, sbp$exercise)

table(sbp$Overweight, sbp$overwt)

table(sbp$Alcohol, sbp$alcohol)

table(sbp$Treatment_Group, sbp$trt)

table(sbp$Stress, sbp$stress)

table(sbp$Salt, sbp$salt)

table(sbp$Childbearing_Potential, sbp$chldbear)

table(sbp$Income, sbp$income)

table(sbp$Education, sbp$educatn)
```
...
Having done some basic data quality checks (above) and some univariate EDA work (two weeks ago), we can now explore our variables conditioned on other variables, i.e., bivariate relationships.  
...
It's fairly difficult to overlay two histograms to get a comparative sense for the two (or more) groups, but a density plot with a suitable alpha value can do so. Use the `fill` option to break the shading of the distributions into groups.  
...
ggplot(sbp, aes(x = weight)) +
  ...
  facet_wrap(~ Exercise, ncol = 1) + # try ncol = 3
  ylab("") +
  theme_bw() +
...
Note that we've now specified that the numeric variable is the *y*-axis and the categorical one is on the *x*-axis. We've also flipped the axes to promote an easier comparison across groups.   
...
There's probably no reason to use a beanplot like this; you could use a faceted density plot and get a nicer look (see below). However, beanplots can be useful if you have a lot of categories and/or you want to see the density *and* a side-by-side comparison, as you can with a boxplot, e.g., 

```{r echo = FALSE, fig.height = 3.5}
set.seed(9)
bean_data = data.frame(Value = sample(1:100, 1200, replace = TRUE), 
...
beanplot(Value ~ Group, data = bean_data, what = c(0, 1, 1, 0), side = "first",
         innerborder = "black", xlab = "", ylab = "Value", cex.axis = 0.7,
         col = c("gray", "transparent", "transparent", "blue"))
```

As mentioned above, a comparison across a few categories could be accomplished more cleanly in `ggplot2` with a comparative density plot using facets, e.g.,  
...
A particular type of numeric/categorical comparisons is a before/after study. While all of the tools above can (and should) be used for that, there are a couple more visualizations that can be useful for a pre/post-type of contrast.
...
2. Create a boxplot of the `bmi` variable, using `Smoke` and `Treatment_Group` as the conditioning variables.  
3. (*Optional*) Swap the *x*-axis/fill assignments for `Smoke` and `Treatment_Group` to produce another boxplot to see what a change in conditioning does to the plot.  
...
```{r fig.height = 3.5, fig.width = 5.25}
# Scatterplot with grouping and loess trends
ggplot(sbp, aes(x = weight, y = sbp, color = Treatment_Group)) +
  geom_point() +
  geom_smooth()
```
...
Things can start getting messy, and even moving into a 3d plot doesn't really help. Later in this course we'll cover dimension reduction to deal with this issue, so that we can adequately visualize multiple dimensions in ways that advance our understanding of the data.  
...
And you can combine them, as we saw above for regaulr bar charts.    
...
You can use the `facet_grid` function in ggplot to show scatterplots faceted by two categorical variables.  
...
The `ggpairs` function from `GGally` provides a way to compare variables using density, scatter, box, and bar plots. We'll use it first only on numeric variables to show a scatterplot matrix with scatterplots on the lower side, densities on the diagonal, and correlations on the upper side (all by default; see the [`ggpairs` website](http://ggobi.github.io/ggally/#ggallyggpairs) for more possibilities).  
...
The real power of `ggpairs` is in being able to compare a variety of different types of variables all at once. Using the `mapping` option allows you to provide ggplot-based aesthetics. You can also adjust the theme by adding it on like you for a regular ggplot.  

```{r fig.height = 7, cache = TRUE}
# Subset for ease of plotting
sbp_both = sbp[ , c(1, 6:8, 13, 22:25)]

# All-of-the-above matrix
ggpairs(sbp_both, columns = c(1, 3, 5:6, 8), mapping = aes(color = Treatment_Group, 
  alpha = 0.4)) +
  theme_bw() +
...
3. (*Optional*) Swap the *x*-axis/fill assisgnments for `Smoke` and `Treatment_Group` to produce another `bmi` boxplot to see what a change in conditioning does to the plot.  
