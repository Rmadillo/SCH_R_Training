...
Once we have a thorough understanding of each variable, we're ready to explore the possible relationships between variables---usually two or three dimensions (sometimes four) can be visualized with a single plot. Matrix plots provide additional ways to compare 1-4 dimensions across several variables.    
...
For the purposes of training, we started with univariate data exploration to get quickly into analysis with R, but in reality you should do some basic quality checks of your data at the start---specifically, explore missing data, duplicated rows, or suspicious values.  
...
Having done some basic data quality checks (above) and some univariate EDA work (two weeks ago & earlier today), we can now explore our variables conditioned on other variables, i.e., bi-/multi- variate relationships.  
